...
async def get_rooms(session: AsyncSession, sort_by: str | None = None, ascending: bool = True):
    query = select(Room)
    if sort_by == "price":
        query = query.order_by(asc(Room.price) if ascending else desc(Room.price))
    elif sort_by == "created_at":
        query = query.order_by(asc(Room.created_at) if ascending else desc(Room.created_at))
